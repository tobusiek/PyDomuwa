...
@router.post("/", status_code=status.HTTP_201_CREATED, response_model=GameView)
...
@router.get("/{game_id}", status_code=status.HTTP_200_OK, response_model=GameView)
...
@router.get("/", status_code=status.HTTP_200_OK, response_model=list[Type[GameView]])
async def get_all_games(db: Session = Depends):
    games = await get_all_objs_of_type(Game, db)
    return [create_game_view(game) for game in games]


@router.put("/add_player", status_code=status.HTTP_200_OK, response_model=GameView)
...
@router.put("/remove_player", status_code=status.HTTP_200_OK, response_model=GameView)
...
@router.delete("/", status_code=status.HTTP_204_NO_CONTENT, response_model=Response)
