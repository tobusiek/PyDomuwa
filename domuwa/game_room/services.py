...
from domuwa.game_room.schema import GameCategory, GameRoomCreate
...
async def create_game_room(game: GameRoomCreate, db: Session = Depends(get_db)) -> GameRoom:
    query_filter = Question.game_name == game.game_name
    if game.game_category != GameCategory.MIXED.value:
        query_filter = query_filter and Question.game_category == game.game_category
...
async def add_player(game_room_id: int, player_id: int, db: Session = Depends(get_db)) -> Type[GameRoom]:
    ...
    game_room = await get_obj_of_type_by_id(game_room_id, GameRoom, "Game", db)
...
async def delete_game_room(game_room_id: int, db: Session = Depends(get_db)) -> None:
    await db_obj_delete(game_room_id, GameRoom, "Game", db)
    players = db.query(Player).filter(Player.game_room_id == game_room_id).all()
