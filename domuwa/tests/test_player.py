...
def post_valid_player(
    ...
    player_data.pop(test_data.SCORE)
...
def test_create_player() -> None:
    ...
    player_response = tests_setup.client.get(PLAYERS_PREFIX + str(player_id))
    assert player_response.status_code == status.HTTP_200_OK, player_response.text

    player_response_data = player_response.json()
    validate_response(player_data, player_response_data)
    ...
    validate_response(player_response_data, db_player.__dict__)


def test_create_player_invalid_data() -> None:
...
def test_update_player_name_invalid_data() -> None:
    player_data = next(post_valid_player())
    player_id = player_data[test_data.ID]
